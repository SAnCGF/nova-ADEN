...
class _SupplierPageState extends State<SupplierPage> {
  ...
  Future<void> _saveSupplier() async {
    if (_nameController?.text.trim().isEmpty ?? true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('⚠️ Nombre requerido')),
      );
      return;
    }
    ...
    try {
      ...
      if (_isEditing && _editingId != null) {
        id = await _repository.updateSupplier(_editingId!, supplier);
        _showMessage('Proveedor actualizado');
      } else {
        id = await _repository.createSupplier(supplier);
